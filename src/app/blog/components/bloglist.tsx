...
export default async function BlogList() {
    ...
    return (
        <div>
            {
                blogs.map((blog: Blog) => (
                    <div key={blog.slug} className="mt-4">
                        <Image src={urlFor(blog.image).url()} 
                        alt={blog.title} 
                        width={800} 
                        height={800} 
                        className="cursor-pointer w-80 h-60 sm:w-72 md:w-96 md:h-64 lg:w-[450px] lg:h-72 transition duration-300 ease-in-out scale-105 shadow-lg dark:hover:shadow-black/30" />
